...
class StatTracker
  ...
  def best_offense
    offense
    best_offense = offense.max_by { |k,v| v}
    best_offense = best_offense[0]
    team = @teams.find { |team| team.team_id.to_i == best_offense }
    team.teamname
  end

  def worst_offense
    offense
    worst_offense = offense.min_by { |k,v| v}
    worst_offense = worst_offense[0] ##Need to convert to team name
    team = @teams.find { |team| team.team_id.to_i == worst_offense }
    team.teamname
  end
  ...
  def winningest_coach(season)
    ...
    best_coach = wins.max_by { |coach, win_percent| win_percent }
    best_coach[0]
  end

  def worst_coach(season)
    wins = group_wins(games_by_coach(season))
    worst_coach = wins.min_by { |coach, win_percent| win_percent }
    worst_coach[0]
  end
  ...
  def rival(team_id)
    ...
    all_rivals = all_teams.map do |team|
      team.first
    end
